...
class Limiter:        
    def startup(self, app: MystbinApp) -> None:
        ...
        self.bucket_cls = RedisLimitBucket if stragegy else InMemoryLimitBucket
...
async def _ignores_ratelimits(request: Request):
    ...
    return bool(request.state.user and request.state.user["admin"])
...
async def ratelimit_zone_key(zone: str, request: Request) -> str:
    ...
    return (
        ("premium_" if user["subscriber"] else "authed_") + zone
...
def ratelimit_id_key(request: Request) -> str:
    ...
    return (
        str(userid)
        if userid
        else request.headers.get("X-Forwarded-For", None)
